...
{	int j, h = now._nr_pr;
	...
	case 10:	/* claim select */
		...
		if (whichclaim == -1 && claimname == NULL)
			printf("0: Claim p1 (4), from state 5\n");
		if (whichclaim != -1)
	...
	case 3:	/* Consumer */
		...
		/* locals: */
		((P3 *)pptr(h))->_10_id = 0;
		((P3 *)pptr(h))->_10_prod = 0;
#ifdef VAR_RANGES
		logval("Consumer:id", ((P3 *)pptr(h))->_10_id);
		logval("Consumer:prod", ((P3 *)pptr(h))->_10_prod);
#endif
#ifdef HAS_CODE
		locinit3(h);
#endif
	...
	case 2:	/* PriorityQueue */
		...
		((P2 *)pptr(h))->_9_id = 0;
#ifdef VAR_RANGES
		logval("PriorityQueue:id", ((P2 *)pptr(h))->_9_id);
#endif
#ifdef HAS_CODE
		locinit2(h);
#endif
	...
	case 1:	/* ProducerB */
		...
		/* params: */
		/* locals: */
		((P1 *)pptr(h))->_8_received = 0;
		((P1 *)pptr(h))->_8_idToSend = 0;
		((P1 *)pptr(h))->_8_idSent = 0;
#ifdef VAR_RANGES
		logval("ProducerB:received", ((P1 *)pptr(h))->_8_received);
		logval("ProducerB:idToSend", ((P1 *)pptr(h))->_8_idToSend);
		logval("ProducerB:idSent", ((P1 *)pptr(h))->_8_idSent);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* ProducerA */
		...
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->_7_received = 0;
		((P0 *)pptr(h))->_7_idToSend = 0;
		((P0 *)pptr(h))->_7_idSent = 0;
#ifdef VAR_RANGES
		logval("ProducerA:received", ((P0 *)pptr(h))->_7_received);
		logval("ProducerA:idToSend", ((P0 *)pptr(h))->_7_idToSend);
		logval("ProducerA:idSent", ((P0 *)pptr(h))->_7_idSent);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(void)
{
		...
		now.cb = addqueue(5, 0);
#ifdef VAR_RANGES
		logval("idToReceiveA", now.idToReceiveA);
		logval("idToReceiveB", now.idToReceiveB);
		logval("i", now.i);
#endif
...
{	int j=0, i = now._nr_qs;
	...
	switch (n) {
	case 5: j = sizeof(Q5); q_flds[5] = 3; q_max[5] = 3; break;
	case 4: j = sizeof(Q4); q_flds[4] = 3; q_max[4] = 3; break;
	case 3: j = sizeof(Q3); q_flds[3] = 3; q_max[3] = 3; break;
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.pq) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.pq) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.pq) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.pq) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.pa) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.pa) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.pa) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.pa) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.pb) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.pb) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.pb) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.pb) )))
			    ) return 0; break;
		case 4: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.ca) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.ca) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.ca) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.ca) )))
			    ) return 0; break;
		case 5: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.cb) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.cb) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.cb) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.cb) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	byte   idToReceiveA:	%d\n", now.idToReceiveA);
	printf("	byte   idToReceiveB:	%d\n", now.idToReceiveB);
	printf("	byte   i:	%d\n", now.i);
	printf("	chan pq (=%d):	len %d:\t", now.pq, q_len(now.pq));
	c_chandump(now.pq);
	printf("	chan pa (=%d):	len %d:\t", now.pa, q_len(now.pa));
	c_chandump(now.pa);
	printf("	chan pb (=%d):	len %d:\t", now.pb, q_len(now.pb));
	c_chandump(now.pb);
	printf("	chan ca (=%d):	len %d:\t", now.ca, q_len(now.ca));
	c_chandump(now.ca);
	printf("	chan cb (=%d):	len %d:\t", now.cb, q_len(now.cb));
	c_chandump(now.cb);
}
void
...
{	/* int i; */
	...
	printf("	byte   id:	%d\n", ((P3 *)pptr(pid))->_10_id);
		break;
	case 2:
		printf("local vars proc %d (PriorityQueue):\n", pid);
	printf("	byte   id:	%d\n", ((P2 *)pptr(pid))->_9_id);
		break;
	case 1:
		printf("local vars proc %d (ProducerB):\n", pid);
	printf("	byte   received:	%d\n", ((P1 *)pptr(pid))->_8_received);
	printf("	byte   idToSend:	%d\n", ((P1 *)pptr(pid))->_8_idToSend);
	printf("	byte   idSent:	%d\n", ((P1 *)pptr(pid))->_8_idSent);
		break;
	case 0:
		printf("local vars proc %d (ProducerA):\n", pid);
	printf("	byte   received:	%d\n", ((P0 *)pptr(pid))->_7_received);
	printf("	byte   idToSend:	%d\n", ((P0 *)pptr(pid))->_7_idToSend);
	printf("	byte   idSent:	%d\n", ((P0 *)pptr(pid))->_7_idSent);
		break;
	}
}
void
...
{
	switch (x) {
	case 1: Printf("Ack"); break;
	case 2: Printf("Msg"); break;
...
{	uchar *z; int slot;
	...
	case 5:
		...
		{	printf(" [");
			...
			printm(((Q5 *)z)->contents[slot].fld1);
	...
	case 4:
		...
		{	printf(" [");
			...
			printm(((Q4 *)z)->contents[slot].fld1);
	...
	case 3:
		...
		{	printf(" [");
			...
			printm(((Q3 *)z)->contents[slot].fld1);
